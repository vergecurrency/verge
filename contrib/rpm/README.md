...
The RPM spec file provided here is for Verge-Core 0.12.0 and builds on CentOS
...
necessary, the packager should bring the problem to the attention of the verge
...
needed when 0.12.1 is released, a proper fix is already in the VERGE git
master branch.
...
idea to build VERGE Core against a static version of that library compiled
...
The `verge.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `verge-server` package. At this point, most RPM based
...
The `verge-server` package should have SELinux support. How to properly do
...
verge-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/verge/). He has been
packaging VERGE for Fedora at least since 2012.
...
4. I renamed the `verge` package that contains the Qt GUI to `verge-core` as
that appears to be how the general population refers to it, in contrast to
`verge-xt` or `verge-classic`. I wanted to make sure the general population
