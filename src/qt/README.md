This directory contains the VERGEQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the VERGE UI.
...
ClientModel is used by the main application `vergegui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/bitcoin/bitcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a verge:... link from a browser).
...
* UI elements like VERGEAmountField, which inherit from QWidget.
* `vergestrings.cpp`: automatically generated
* `vergeunits.(h/cpp)`: XVG / mXVG / etc handling
* `callback.h`
...
* don't change `local/verge_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
4. Enter "verge-qt" as project name, enter src/qt as location
