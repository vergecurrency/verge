...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = strprintf("%s verge-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
            "Usage:\n"
              "  verge-tx [options] <hex-tx> [commands]  Update hex-encoded VERGE transaction\n" +
              "  verge-tx [options] -create [commands]   Create hex-encoded VERGE transaction\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (!IsValidDestination(destination) || destination.type() == typeid(CNotAStealthAddress)) {
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded verge transaction
