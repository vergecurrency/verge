    ...
    while (nHeight < nHeightEnd && !ShutdownRequested())
    {
        ...
        if (!SignBlock(*pblock, *pwallet)) {
            throw JSONRPCError(RPC_WALLET_ERROR,
                               "Failed to sign generated block");
        }
...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) A wallet-owned address to send the newly generated VERGE to.\n"
    ...
    const CKeyID* key_id = boost::get<CKeyID>(&destination);
    if (!key_id) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                           "Mining address must refer to a legacy key");
    }
    ...
    CKey key;
    {
        ...
        if (!pwallet->GetKey(*key_id, key)) {
            throw JSONRPCError(
                RPC_WALLET_ERROR,
                "Mining address must be controlled by this wallet");
        }
    }
...
static CKeyID ParseStakingDestination(const UniValue& value,
                                      const char* argument)
{
    const CTxDestination destination = DecodeDestination(value.get_str());
    if (!IsValidDestination(destination) ||
        destination.type() != typeid(CKeyID)) {
        throw JSONRPCError(
            RPC_INVALID_ADDRESS_OR_KEY,
            strprintf("%s must be a legacy key-hash address", argument));
    }
    return boost::get<CKeyID>(destination);
}

static UniValue createbond(const JSONRPCRequest& request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CWallet* const pwallet = wallet.get();
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) return NullUniValue;
    if (request.fHelp || request.params.empty() || request.params.size() > 3) {
        throw std::runtime_error(
            "createbond amount ( \"reward_address\" \"withdrawal_address\" )\n"
            "\nCreates and broadcasts an explicit proof-of-stake bond. The "
            "delegated signing key is encrypted by the wallet. Omitted owner "
            "addresses are generated by this wallet.\n"
            "\nArguments:\n"
            "1. amount                 (numeric, required) amount to bond\n"
            "2. reward_address         (string, optional) fee reward address\n"
            "3. withdrawal_address     (string, optional) principal owner\n");
    }
    const CAmount amount = AmountFromValue(request.params[0]);
    if (amount < Params().GetConsensus().nPoSMinStake) {
        throw JSONRPCError(RPC_INVALID_PARAMETER,
                           "Bond amount is below the minimum stake");
    }
    ...
    const CPubKey staking_pubkey = pwallet->GenerateNewKey(batch);
    CKey staking_key;
    if (!pwallet->GetKey(staking_pubkey.GetID(), staking_key)) {
        throw JSONRPCError(RPC_WALLET_ERROR,
                           "Failed to retrieve delegated staking key");
    }

    auto owner_key = [&](size_t parameter, const char* label) {
        if (request.params.size() > parameter &&
            !request.params[parameter].isNull()) {
            const CKeyID key_id = ParseStakingDestination(
                request.params[parameter], label);
            if (!pwallet->HaveKey(key_id)) {
                throw JSONRPCError(
                    RPC_WALLET_ERROR,
                    strprintf("%s must be controlled by this wallet", label));
            }
            return key_id;
        }
        const CPubKey key = pwallet->GenerateNewKey(batch);
        const CTxDestination destination = GetDestinationForKey(
            key, OutputType::LEGACY);
    ...
    if (!keys_ok) {
        throw JSONRPCError(RPC_WALLET_ERROR,
                           "Failed to derive delegated staking credentials");
    }
    bond.reward_key_id = owner_key(1, "staking rewards");
    bond.withdrawal_key_id = owner_key(2, "staking withdrawal");
    if (!pos::IsValid(bond)) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Generated invalid bond data");
    }

    std::vector<CRecipient> recipients{
        CRecipient{pos::GetBondScript(bond), amount, false}};
    CReserveKey change_key(pwallet);
    CAmount fee = 0;
    int change_position = -1;
    std::string error;
    CTransactionRef tx;
    CCoinControl coin_control;
    coin_control.m_min_depth = Params().GetConsensus().nPoSStakeMaturity;
    if (!pwallet->CreateTransaction(recipients, tx, change_key, fee,
    ...
    if (!pwallet->CommitTransaction(tx, std::move(metadata), {}, "",
                                    change_key, g_connman.get(),
                                    validation_state) ||
        !validation_state.IsValid()) {
        throw JSONRPCError(
            RPC_WALLET_ERROR,
            strprintf("Bond transaction rejected: %s",
    ...
    for (size_t i = 0; i < tx->vout.size(); ++i) {
        if (tx->vout[i].scriptPubKey == pos::GetBondScript(bond)) {
            result.pushKV("vout", static_cast<int>(i));
            break;
        }
    }
    result.pushKV("amount", ValueFromAmount(amount));
    result.pushKV("reward_address",
                  EncodeDestination(CKeyID(bond.reward_key_id)));
    result.pushKV("withdrawal_address",
                  EncodeDestination(CKeyID(bond.withdrawal_key_id)));
    return result;
}

static UniValue unbond(const JSONRPCRequest& request)
{
    std::shared_ptr<CWallet> const wallet = GetWalletForJSONRPCRequest(request);
    CWallet* const pwallet = wallet.get();
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) return NullUniValue;
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            "unbond \"txid\" vout\n"
            "\nBegins withdrawal of a wallet-owned bond. The complete "
            "principal is preserved in a consensus-locked unbond output; "
            "transaction fees are paid by other wallet inputs.\n");
    }
    ...
    if (coin.IsSpent()) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                           "Bond output does not exist or is already spent");
    }
    pos::BondData bond;
    if (!pos::ParseBondScript(coin.out.scriptPubKey, bond)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER,
                           "Selected output is not a proof-of-stake bond");
    }
    if (!pwallet->HaveKey(CKeyID(bond.withdrawal_key_id))) {
        throw JSONRPCError(RPC_WALLET_ERROR,
                           "Wallet does not control the withdrawal key");
    }
    const int64_t unlock_height = static_cast<int64_t>(chainActive.Height()) +
        1 + Params().GetConsensus().nPoSUnbondingBlocks;
    if (unlock_height > std::numeric_limits<uint32_t>::max()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Unbond height is out of range");
    }
    pos::UnbondData unbond_data;
    unbond_data.unlock_height = static_cast<uint32_t>(unlock_height);
    unbond_data.withdrawal_key_id = bond.withdrawal_key_id;

    std::vector<CRecipient> recipients{
        CRecipient{pos::GetUnbondScript(unbond_data), coin.out.nValue, false}};
    CCoinControl coin_control;
    ...
    CValidationState validation_state;
    if (!pwallet->CommitTransaction(tx, {}, {}, "", change_key,
                                    g_connman.get(), validation_state)) {
        throw JSONRPCError(
            RPC_WALLET_ERROR,
            strprintf("Unbond transaction rejected: %s",
                      FormatStateMessage(validation_state)));
    }
    UniValue result(UniValue::VOBJ);
    result.pushKV("txid", tx->GetHash().GetHex());
    result.pushKV("unlock_height", unlock_height);
    result.pushKV("amount", ValueFromAmount(coin.out.nValue));
    return result;
}
...
static UniValue generatestake(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !request.params.empty()) {
        throw std::runtime_error(
            "generatestake\n"
            "\nEvaluates this wallet's bonds for the current canonical slot "
            "and submits one proof-of-stake block when eligible.\n");
    }
    uint256 block_hash;
    std::string error;
    if (!TryStakeBlock(*pwallet, block_hash, error, true)) {
...
static UniValue getstakinginfo(const JSONRPCRequest& request)
{
    ...
    result.pushKV("initial_snapshot_value",
                  ValueFromAmount(initial_snapshot == nullptr
                                      ? 0
                                      : initial_snapshot->total_value));
    return result;
}
...
{ //  category              name                                actor (function)                argNames
    ...
    { "staking",            "createbond",                       &createbond,                    {"amount","reward_address","withdrawal_address"} },
