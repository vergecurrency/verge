...
sudo apt-get --yes -qq install software-properties-common
sudo add-apt-repository --yes ppa:ubuntu-sdk-team/ppa
sudo add-apt-repository --yes ppa:bitcoin/bitcoin
sudo apt-get update -qq

# add the cross compiling stuffs
sudo apt-get --yes -qq install dpkg-dev git sudo make mingw-w64 mingw-w64-common wget g++-mingw-w64 binutils-mingw-w64-x86-64 g++-mingw-w64-x86-64 gcc-mingw-w64-x86-64 mingw-w64-tools mingw-w64-x86-64-dev zip

# base requirements for building wallet (pulled from linux)
sudo apt-get --yes -qq install build-essential libtool autotools-dev automake pkg-config libssl-dev libevent-dev bsdmainutils git libboost-all-dev libminiupnpc-dev libprotobuf-dev protobuf-compiler libqrencode-dev protobuf-compiler autoconf bsdmainutils python
